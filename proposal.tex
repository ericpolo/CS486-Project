...
%%%%%%%%%. Result %%%%%%%%%
\section{Result}
After running both CNN and RNN algorithms on the Fashion-MNIST dataset, we have obtained the following result. In this section, we will compare and contrast the results obtained for CNN and RNN in three aspects, test accuracy, time efficiency, and memory usage.

The first important performance indicator of a neural network is its test accuracy on test set. We run both algorithms with 30 epochs on both test environments, Windows and Mac OS. The graph below shows the change of validation accuracy over number of epochs. Test accuracy of CNN finally reaches 0.9274 on Windows environment and 0.9303 on MacOS environment  Instead, test accuracy of RNN only reaches 0.5328 on Windows environment and 0.6083 on MacOS environment. By shown in Figure 3, Figure 4 and data above, we can clearly observe that CNN has significant advantage on accuracy over RNN when we run both algorithms with the same number of epochs. On the other hand, test accuracy of RNN can hardly reaches 0.7335 even after running 200 epochs, as shown in the Figure 5.
